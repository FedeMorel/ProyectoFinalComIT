...
const returnAllItems = async(req = request, res = response) => {
    ...
    if (tematica) {
        const [total, colecciones] = await Promise.all([Producto.countDocuments({ "categoria": "Country", "subcategoria": "Letras", "tematica": tematica }), Producto.find({ "categoria": "Country", "subcategoria": "Letras", "tematica": tematica })]);
        res.json({
            "cantidad": total,
            "resultados": colecciones
        });
    } else {
        const [total, colecciones] = await Promise.all([Producto.countDocuments({ "categoria": "Country", "subcategoria": "Letras" }), Producto.find({ "categoria": "Country", "subcategoria": "Letras" })]);
        res.json({
            "cantidad": total,
            "resultados": colecciones
        });
