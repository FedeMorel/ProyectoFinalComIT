...
const returnAllItems = async(req = request, res = response) => {
    ...
    if (tematica) {
        const [total, colecciones] = await Promise.all([Producto.countDocuments({ "categoria": "Country", "subcategoria": "Bebé", "tematica": tematica }), Producto.find({ "categoria": "Country", "subcategoria": "Bebé", "tematica": tematica })]);
        res.json({
            "cantidad": total,
            "resultados": colecciones
        });
    } else {
        const [total, colecciones] = await Promise.all([Producto.countDocuments({ "categoria": "Country", "subcategoria": "Bebé" }), Producto.find({ "categoria": "Country", "subcategoria": "Bebé" })]);
        res.json({
            "cantidad": total,
            "resultados": colecciones
        });
