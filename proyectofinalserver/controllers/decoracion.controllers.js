...
const returnAllItems = async(req = request, res = response) => {
    ...
    if (tematica) {
        const [total, colecciones] = await Promise.all([Producto.countDocuments({ "categoria": "Country", "subcategoria": "Decoración", "tematica": tematica }), Producto.find({ "categoria": "Country", "subcategoria": "Decoración", "tematica": tematica })]);
        res.json({
            "cantidad": total,
            "resultados": colecciones
        });
    } else {
        const [total, colecciones] = await Promise.all([Producto.countDocuments({ "categoria": "Country", "subcategoria": "Decoración" }), Producto.find({ "categoria": "Country", "subcategoria": "Decoración" })]);
        res.json({
            "cantidad": total,
            "resultados": colecciones
        });
