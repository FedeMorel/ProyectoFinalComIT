...
const returnAllItems = async(req = request, res = response) => {
    ...
    if (tematica) {
        const [total, colecciones] = await Promise.all([Producto.countDocuments({ "categoria": "Country", "subcategoria": "Cocina", "tematica": tematica }), Producto.find({ "categoria": "Country", "subcategoria": "Cocina", "tematica": tematica })]);
        res.json({
            "cantidad": total,
            "resultados": colecciones
        });
    } else {
        const [total, colecciones] = await Promise.all([Producto.countDocuments({ "categoria": "Country", "subcategoria": "Cocina" }), Producto.find({ "categoria": "Country", "subcategoria": "Cocina" })]);
        res.json({
            "cantidad": total,
            "resultados": colecciones
        });
